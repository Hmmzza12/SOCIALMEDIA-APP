...
// Highlight @mentions in comment text and link to user profile
const renderWithMentions = (text) =>
    text.split(/(@\w+)/g).map((part, i) =>
        part.startsWith('@') && part.length > 1
            ? <Link key={i} to={`/u/${part.slice(1)}`} className="text-accent font-semibold hover:underline" onClick={(e) => e.stopPropagation()}>{part}</Link>
            : part
    );
                ...
                {comments.map(comment => (
                    <div key={comment.id} className="post-card" style={{ cursor: 'default', borderBottom: '1px solid var(--border)' }}>
                        ...
                        <div className="post-content" style={{ marginTop: '-20px' }}>
                            <p className="post-body">{renderWithMentions(comment.content)}</p>
                        </div>
                    </div>
                ))}
                {comments.length === 0 && (
