...
router.get('/by-username/:username', optionalAuth, async (req: Request, res: Response): Promise<void> => {
    try {
...
router.get('/:id', optionalAuth, async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (req.user && req.user.userId !== userId) {
            const [rel] = await db.query<RowDataPacket[]>(
...
router.post('/:id/follow', authenticateToken, async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        await db.query(
            'INSERT INTO notifications (user_id, actor_id, type, post_id) VALUES (?, ?, ?, NULL)',
