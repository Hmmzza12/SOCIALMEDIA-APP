    ...
    try {
        ...
        if (posts[0].user_id !== req.user!.userId) {
            await db.query(
                ...
                [posts[0].user_id, req.user!.userId, 'comment', postId]
            );
        }

        // @mention notifications — notify each mentioned user (skip self & post owner)
        const mentioned = Array.from(new Set((content.match(/@(\w+)/g) || []).map((m: string) => m.slice(1))));
        for (const uname of mentioned) {
            ...
            if (urows.length > 0) {
                const mid = urows[0].id;
                if (mid !== req.user!.userId && mid !== posts[0].user_id) {
                    await db.query(
                        'INSERT INTO notifications (user_id, actor_id, type, post_id) VALUES (?, ?, ?, ?)',
                        [mid, req.user!.userId, 'mention', postId]
                    );
                }
            }
        }
